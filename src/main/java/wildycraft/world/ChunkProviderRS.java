...
public class ChunkProviderRS implements IChunkProvider {
	/** RNG. */
	private Random rand;
					...
					for (int var31 = 0; var31 < 8; ++var31) {
						...
						for (int var42 = 0; var42 < 4; ++var42) {
							...
							for (int var51 = 0; var51 < 4; ++var51) {
								if ((var47 += var49) > 0.0D) {
									par3ArrayOfByte[var43 += var44] = Blocks.stone;
	...
	public void replaceBlocksForBiome(int par1, int par2,
		...
		ChunkProviderEvent.ReplaceBiomeBlocks event = new ChunkProviderEvent.ReplaceBiomeBlocks(this, par1, par2, blockStorage, biomes);
	    MinecraftForge.EVENT_BUS.post(event);
	    if (event.getResult() == Result.DENY) return;

	    double d0 = 0.03125D;
	    this.stoneNoise = this.noiseGen1.generateNoiseOctaves(this.stoneNoise, par1 * 16, par2 * 16, 16, 16, d0 * 2.0D, d0 * 2.0D, 1.0D);
	    ...
	    for (int z = 0; z < 16; z++)
	    {
	      for (int x = 0; x < 16; x++)
	      {
	        ...
	        biomegenbase.genTerrainBlocks(this.worldObj, this.rand, blockStorage, metaStorage, par1 * 16 + z, par2 * 16 + x, this.stoneNoise[(x + z * 16)]);
	      }
	    }
	}
		...
		if (this.parabolicField == null) {
			...
			for (int var8 = -2; var8 <= 2; ++var8) {
				for (int var9 = -2; var9 <= 2; ++var9) {
					float var10 = 10.0F / MathHelper.sqrt_float((float) (var8
							* var8 + var9 * var9) + 0.2F);
		...
		for (int var14 = 0; var14 < par5; ++var14) {
			for (int var15 = 0; var15 < par7; ++var15) {
				...
				for (int var46 = 0; var46 < par6; ++var46) {
					...
					double var30 = 0.0D;
					double var32 = ((double) var46 - var28) * 12.0D * 128.0D
							/ 128.0D / var26;
	...
	public void populate(IChunkProvider par1IChunkProvider, int par2, int par3) {
		...
		if (TerrainGen.populate(par1IChunkProvider, worldObj, rand, par2, par3,
				var11, LAVA) && !var11 && this.rand.nextInt(8) == 0) {
			var12 = var4 + this.rand.nextInt(16) + 8;
		...
		/*for (int k = 0; k < 50; k++) {
			if (TerrainGen.populate(par1IChunkProvider, worldObj, rand, par2,
					par3, var11, LAKE)) {
				int Xcoord1 = var4 + rand.nextInt(16);
				int Ycoord1 = 50 + rand.nextInt(20);
				int Zcoord1 = var5 + rand.nextInt(16);
				(new WorldGenRSTrees(true)).generate(this.worldObj, this.rand,
		...
		for (int k = 0; k < 50; k++) {
			if (TerrainGen.populate(par1IChunkProvider, worldObj, rand, par2,
				...
				int Ycoord1 = 50 + rand.nextInt(20);
				int Zcoord1 = var5 + rand.nextInt(16);
				(new WorldGenRSHut(true)).generate(this.worldObj, this.rand,
						Xcoord1, Ycoord1, Zcoord1);
			}
		}
		if (TerrainGen.generateOre(worldObj, rand,(new WorldGenMinable(Wildycraft.runeOre.blockID, 6)) ,par2, par3, null)) {
			int Xcoord1 = var4 + rand.nextInt(5);
			int Ycoord1 = rand.nextInt(80);
			int Zcoord1 = var5 + rand.nextInt(5);
			(new WorldGenMinable(Wildycraft.runeOre.blockID, 6)).generate(
					this.worldObj, this.rand, Xcoord1, Ycoord1, Zcoord1);
		}

		for (int k = 0; k < 10; k++) {
			if (TerrainGen.populate(par1IChunkProvider, worldObj, rand, par2,
					par3, var11, LAKE)) {
				int Xcoord2 = var4 + rand.nextInt(5);
				int Ycoord2 = rand.nextInt(80);
				int Zcoord2 = var5 + rand.nextInt(5);
				(new WorldGenBloodwoodGiantTree(true)).generate(this.worldObj,
						this.rand, Xcoord2, Ycoord2, Zcoord2);
			}
		}
		for (int k = 0; k < 10; k++) {
			if (TerrainGen.generateOre(worldObj, rand,(new WorldGenMinable(Wildycraft.addyOre.blockID, 50)) ,par2, par3, null)) {
				int Xcoord2 = var4 + rand.nextInt(5);
				int Ycoord2 = rand.nextInt(80);
				int Zcoord2 = var5 + rand.nextInt(5);
				(new WorldGenMinable(Wildycraft.addyOre.blockID, 50)).generate(
						this.worldObj, this.rand, Xcoord2, Ycoord2, Zcoord2);
			}
		}*/
		...
		for (var12 = 0; doGen && var12 < 16; ++var12) {
			for (var13 = 0; var13 < 16; ++var13) {
				...
				if (this.worldObj.isBlockFreezable(var12 + var4, var14 - 1,
						var13 + var5)) {
					this.worldObj.setBlock(var12 + var4, var14 - 1,
							var13 + var5, Blocks.ice);
				}

				if (this.worldObj.func_147478_e(var12 + var4, var14, var13 + var5, true)) {
					this.worldObj.setBlock(var12 + var4, var14, var13
							+ var5, Blocks.snow_layer);
				}
			}
		}
	 ...
	 */
	public ChunkPosition func_147416_a(World par1World, String par2Str,
			int par3, int par4, int par5) {
		return "Stronghold".equals(par2Str) && this.strongholdGenerator != null ? this.strongholdGenerator
				.func_151545_a(par1World, par3, par4, par5) : null;
	}
