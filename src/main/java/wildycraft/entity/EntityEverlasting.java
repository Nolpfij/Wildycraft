...
public class EntityEverlasting extends EntityMob implements IBossDisplayData, EntityDreamMonster{
	...
	private static final IEntitySelector attackEntitySelector = new IEntitySelector()
    {
	 ...
	 public EntityEverlasting(World par1World){
	     super(par1World);
	 ...
	 public void onDeath(DamageSource par1DamageSource) {
			...
			if(!this.worldObj.isRemote){
				EntityUntouchable c = new EntityUntouchable(this.worldObj);
				...
				for(EntityPlayer player: a){
					player.addChatMessage(new ChatComponentText("Can you overcome the challenge?..."));
				}
			}
	 ...
	 protected void dropFewItems(boolean par1, int par2, DamageSource par1DamageSource) {
			...
			this.dropItem(Wildycraft.astralShards, 4 + rand.nextInt(3));
			this.dropItem(Wildycraft.lunarLeather, 6 + rand.nextInt(4));
			...
			for (var4 = 0; var4 < var3; ++var4) {
				...
				if (j > 93) {
					...
					this.dropItem(Item.getItemFromBlock(Wildycraft.brazier), 2);
				} else if (j > 40) {
					this.dropItem(Wildycraft.swordfishCooked, 10);
				} else if (j > 20) {
					this.dropItem(Item.getItemFromBlock(Blocks.diamond_block), 3);
				} else if (j > 10){
					this.entityDropItem(new ItemStack(Items.golden_apple,2,0), 0);
				} else {
					this.dropItem(Items.emerald, rand.nextInt(5) + 5);
				}
			}
		}
	 ...
	 public boolean attackEntityFrom(DamageSource par1DamageSource, float par2){
		 ...
		 if((this.getHealth() < 100 || this.getHealth() - par2 < 0) && getType() != 1){
