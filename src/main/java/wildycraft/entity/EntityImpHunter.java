...
public class EntityImpHunter extends EntityZombie {
	...
	public EntityImpHunter(World par1World) {
		super(par1World);
	...
	protected Entity findPlayerToAttack() {
		...
		List var1 = this.worldObj.getEntitiesWithinAABB(EntityImp.class, par2AxisAlignedBB );
		double var9 = -1.0D;
        Entity var11 = null;
		...
		if (!(par1DamageSource.isFireDamage() || par1DamageSource.getDamageType().equals("cactus"))) {
			if (this.isEntityInvulnerable()) {
				...
				for (int var5 = 0; var5 < var4.size(); ++var5) {
					...
					if (var6 instanceof EntityImpHunter) {
						EntityImpHunter var7 = (EntityImpHunter) var6;
	...
	public void onDeath(DamageSource par1DamageSource) {
		...
		if (par1DamageSource.getEntity() instanceof EntityPlayer){
			...
			//ep.addStat(Nolpcraft.BlackKnight, 1);
		}
	}
	...
	protected void dropFewItems(boolean par1, int par2,DamageSource par1DamageSource) {
		...
		for (var4 = 0; var4 < var3; ++var4) {
			...
			if (i > 90) {
				this.dropItem(Items.bread, 10);
			} else if (i > 60) {
				this.dropItem(Wildycraft.addyspade, 1);
			} else {
