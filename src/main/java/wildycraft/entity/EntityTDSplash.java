...
public class EntityTDSplash extends Entity implements IProjectile
{
    private int xTile = -1;
    private int yTile = -1;
    private int zTile = -1;
    ...
    public EntityTDSplash(World par1World)
    {
        super(par1World);
    ...
    public EntityTDSplash(World par1World, double par2, double par4, double par6)
    {
        super(par1World);
        this.renderDistanceWeight = 10.0D;
        this.setSize(0.5F, 0.5F);
    ...
    public EntityTDSplash(World par1World, EntityLivingBase par2EntityLiving, EntityLivingBase par1EntityLiving, float par4, float par5)
    {
        super(par1World);
        this.renderDistanceWeight = 10.0D;
    ...
    public EntityTDSplash(World par1World, EntityLivingBase par2EntityLiving, float par3)
    {
        super(par1World);
        this.renderDistanceWeight = 10.0D;
        this.shootingEntity = par2EntityLiving;
            ...
            if (var4 != null)
            {
                ...
                else
                {
					...
					List attackRange = this.worldObj.getEntitiesWithinAABBExcludingEntity(
						this, this.boundingBox.expand(3.0D, 3.0D, 3.0D));

					for (int i = 0; i < attackRange.size(); ++i) {
						Entity v6 = (Entity) attackRange.get(i);

						if (v6.posY < this.posY + 0.5) {
							DamageSource ds = new EntityDamageSourceIndirect("Splash",
								this, v6){
									public IChatComponent func_151519_b(EntityLivingBase par1EntityLivingBase){
										String s =  par1EntityLivingBase.getCommandSenderName();
										s += " was erased by the heat";
								    return new ChatComponentText(s);
								}
							}.setProjectile();
							v6.attackEntityFrom(ds, (float)this.getDamage());
						}
					}
					 for (var9 = 0; var9 < 32; ++var9)
		             {
		                    this.worldObj.spawnParticle("lava", this.posX + Math.random() * 4 - 2, this.posY + Math.random() * 4 - 2, this.posZ + Math.random() * 4 - 2, 0, 0, 0);
		             }
					//TODO
