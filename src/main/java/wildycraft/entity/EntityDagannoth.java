...
public class EntityDagannoth extends EntityZombie {
	...
	public EntityDagannoth(World par1World) {
		super(par1World);
		this.isImmuneToFire = false;
		// this.setCurrentItemOrArmor(0, new ItemStack(Nolpcraft.runesword));
	...
	public boolean getCanSpawnHere() {
		...
		Block block = worldObj.getBlock((int)this.posX,(int)(this.posY - 1),(int) this.posZ);
		
		return (block != Blocks.planks && block != Blocks.gravel && block != Blocks.hay_block) &&
				this.worldObj.difficultySetting.getDifficultyId() > 0
	...
	protected Entity findPlayerToAttack() {
		AxisAlignedBB par2AxisAlignedBB = this.boundingBox.getBoundingBox(
				posX - 16, posY - 16, posZ - 16, posX + 16, posY + 16,
				posZ + 16);
		List var1 = this.worldObj.getEntitiesWithinAABB(EntityLivingBase.class,
				par2AxisAlignedBB);
		double var9 = -1.0D;
		Entity var11 = null;

		for (int var12 = 0; var12 < var1.size(); ++var12) {
			if (!(var1.get(var12) instanceof EntityDagannoth)) {
				if (!(var1.get(var12) instanceof EntityRockCrab) && !(var1.get(var12) instanceof EntityDagannothMother) && !(var1.get(var12) instanceof EntityBlueDragon)) {
					Entity var13 = (Entity) var1.get(var12);
					if (var1.get(var12) instanceof EntityPlayer) {
						EntityPlayer var14 = (EntityPlayer) var1.get(var12);

						if (!var14.capabilities.disableDamage) {
							if (var14.isEntityAlive()) {
								double var15 = var13.getDistanceSq(posX, posY,
										posZ);
								double var16 = 16;
		...
		if (!(par1DamageSource.isFireDamage() || par1DamageSource
			...
			if (this.isEntityInvulnerable()) {
				...
				for (int var5 = 0; var5 < var4.size(); ++var5) {
					...
					if (var6 instanceof EntityDagannoth) {
						EntityDagannoth var7 = (EntityDagannoth) var6;
	...
	protected void dropFewItems(boolean par1, int par2,
		...
		for (var4 = 0; var4 < var3; ++var4) {
			...
			if (i > 90) {
				if (par1DamageSource.getEntity() instanceof EntityPlayer) {
					EntityPlayer ep = (EntityPlayer) par1DamageSource
							.getEntity();
					ep.addStat(Wildycraft.GoodAxe, 1);
				}
				this.dropItem(Wildycraft.dragonaxe, 1);
			} else if (i > 60) {
				this.dropItem(Items.diamond_axe, 1);
			}else if (i > 40) {
				this.dropItem(Wildycraft.fremennikBlade, 1);
			} else {
