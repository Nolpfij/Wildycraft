...
public class EntityAirBlast extends Entity implements IProjectile
{
    private int xTile = -1;
    private int yTile = -1;
    private int zTile = -1;
    ...
    public EntityAirBlast(World par1World, EntityLivingBase par2EntityLiving, EntityLivingBase par3EntityLiving, float par4, float par5)
    {
        super(par1World);
        this.renderDistanceWeight = 10.0D;
    ...
    public EntityAirBlast(World par1World, EntityLivingBase par2EntityLiving, float par3)
    {
        super(par1World);
        this.renderDistanceWeight = 10.0D;
        this.shootingEntity = par2EntityLiving;
    ...
    public void onUpdate()
    {
        ...
        else
        {
            ...
            if (var4 != null)
            {
                if (var4.entityHit != null)
                {
                    ...
                    if(var4.entityHit instanceof EntityPlayer){
                		EntityPlayer temp4 = (EntityPlayer)var4.entityHit;
                		if(this.shootingEntity instanceof EntityDreadFowl)
                		{
                			EntityDreadFowl temp5 = (EntityDreadFowl)this.shootingEntity;
                			if(temp4.getCommandSenderName().equalsIgnoreCase(temp5.getOwnerName())){
                				owner = true;
                			}
                		}
                    ...
                    boolean ally = false;
                    if(var4.entityHit instanceof RSFamiliar){
                    	RSFamiliar temp4 = (RSFamiliar)var4.entityHit;
                		if(this.shootingEntity instanceof RSFamiliar)
                		{
                			RSFamiliar temp5 = (RSFamiliar)this.shootingEntity;
                			if(temp4.sameOwner(temp5)){
                				ally = true;
                			}
                		}
                    ...
                    if(!owner && !ally){
                    	if (this.shootingEntity == null){
                				var21 = Wildycraft.causeAirBlastDamage(this, this);
                			}
                			else
                			{
                				var21 = Wildycraft.causeAirBlastDamage(this, this.shootingEntity);
                			}
                    ...
                    if (var4.entityHit.attackEntityFrom(var21, var23))
                    {
                        if (var4.entityHit instanceof EntityLivingBase)
                        {
                            ...
                            if (this.knockbackStrength > 0)
                            {
                                ...
                                if (var26 > 0.0F)
                                {
                                    var4.entityHit.addVelocity(this.motionX * (double)this.knockbackStrength * 0.6000000238418579D / (double)var26, 0.5D, this.motionZ * (double)this.knockbackStrength * 0.6000000238418579D / (double)var26);
                                }
                            }
            ...
            //{
                ...
                {
                    this.worldObj.spawnParticle("enchantmenttable", this.posX + this.motionX * (double)var9 / 4.0D, this.posY + this.motionY * (double)var9 / 4.0D, this.posZ + this.motionZ * (double)var9 / 4.0D, -this.motionX, -this.motionY + 0.2D, -this.motionZ);
                }
            //}
