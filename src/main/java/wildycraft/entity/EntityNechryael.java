...
public class EntityNechryael extends EntityMob{
	...
	private boolean persistenceRequired = false;
	 ...
	 public EntityNechryael(World par1World){
	     super(par1World);
	 ...
	 {
		 if(getSummonDelay() > 0){
			 setSummonDelay(getSummonDelay()-1);
		 }
	 ...
	 protected void dropFewItems(boolean par1, int par2,
			...
			for (var4 = 0; var4 < var3; ++var4) {
				...
				if (i > 85) {
					this.dropItem(Items.bone, rand.nextInt(2) + 4);
				} else if (i > 65){
					this.dropItem(Wildycraft.swordfishRaw,3);
				}else if (i > 45){
					this.dropItem(Item.getItemFromBlock(Blocks.gold_ore),5);
				}else if (i > 37){
					this.dropItem(Items.diamond,1);
				}else if (i > 29){
					this.dropItem(Items.golden_apple,1);
				}else if (i > 8){
					this.dropItem(Items.experience_bottle,7);
				}else {
					this.dropItem(Wildycraft.darkMysticRobeTop,1);
				}
			}
		}
	 ...
	 public void setSummonDelay(int par1){
	 ...
	 public boolean attackEntityFrom(DamageSource par1DamageSource, float par2){
		 if(!this.worldObj.isRemote){
			 if(getSummonDelay() == 0){
				 EntityDeathSpawn c = new EntityDeathSpawn(this.worldObj);
