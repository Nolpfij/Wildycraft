...
public class EntityBaitedFishHook extends EntityFishHook implements IEntityAdditionalSpawnData
{
    ...
    public EntityBaitedFishHook(World par1World)
    {
        super(par1World);
        this.xTile = -1;
        this.yTile = -1;
    ...
    @SideOnly(Side.CLIENT)
    public EntityBaitedFishHook(World par1World, double par2, double par4, double par6, EntityPlayer par8EntityPlayer)
    {
        this(par1World);
    ...
    public EntityBaitedFishHook(World par1World, EntityPlayer par2EntityPlayer)
    {
        super(par1World);
        this.xTile = -1;
        this.yTile = -1;
    ...
    public void onUpdate()
    {
        ...
        else
        {
            if (!this.worldObj.isRemote)
            {
                ...
                if (this.field_146042_b.isDead || !this.field_146042_b.isEntityAlive() || itemstack == null || itemstack.getItem() != Wildycraft.baitedFishingRod || this.getDistanceSqToEntity(this.field_146042_b) > 1024.0D)
                {
                    this.setDead();
    ...
    public int catchFish()
    {
        ...
        else
        {
            ...
            if (this.field_146043_c != null)
            {
                ...
                double d4 = 0.1D;
                this.field_146043_c.motionX += d0 * d4;
                this.field_146043_c.motionY += d1 * d4 + (double)MathHelper.sqrt_double(d3) * 0.08D;
                this.field_146043_c.motionZ += d2 * d4;
                b0 = 3;
            }
            else if (this.ticksCatchable > 0)
            {
            	if(this.field_146042_b.capabilities.isCreativeMode || this.field_146042_b.inventory.hasItem(Wildycraft.fishingBait)){
            		
            		EntityItem entityitem;
            		int k = rand.nextInt(100);
            		if (k > 50){
            			entityitem = new EntityItem(this.worldObj, this.posX, this.posY, this.posZ, new ItemStack(Wildycraft.herringRaw));
            		} else {
            			if (k < 10 && this.field_146042_b.posY < 20){
            				entityitem = new EntityItem(this.worldObj, this.posX, this.posY, this.posZ, new ItemStack(Wildycraft.cavefishRaw));
            			} else {
            				entityitem = new EntityItem(this.worldObj, this.posX, this.posY, this.posZ, new ItemStack(Wildycraft.pikeRaw));
            			}
            		}
                	...
                	if(!this.field_146042_b.capabilities.isCreativeMode){
                		this.field_146042_b.inventory.consumeInventoryItem(Wildycraft.fishingBait);
                	}
