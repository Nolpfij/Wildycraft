...
public class EntityWhiteKnight extends EntityZombie {
	...
	protected Entity findPlayerToAttack() {
		...
		List var1 = this.worldObj.getEntitiesWithinAABB(EntityBlackKnight.class, par2AxisAlignedBB );
		double var9 = -1.0D;
        Entity var11 = null;
		...
		if (!(par1DamageSource.isFireDamage() || par1DamageSource.getDamageType().equals("cactus"))) {
			if (this.isEntityInvulnerable()) {
				...
				for (int var5 = 0; var5 < var4.size(); ++var5) {
					...
					if (var6 instanceof EntityWhiteKnight) {
						EntityWhiteKnight var7 = (EntityWhiteKnight) var6;
	...
	public void onDeath(DamageSource par1DamageSource) {
		...
		if (par1DamageSource.getEntity() instanceof EntityPlayer){
			...
			ep.addStat(Wildycraft.BlackKnight, 1);
		}
	}
	...
	protected void dropFewItems(boolean par1, int par2,DamageSource par1DamageSource) {
		...
		for (var4 = 0; var4 < var3; ++var4) {
			...
			if (i > 70) {
				...
				if (j > 75){
					this.dropItem(Wildycraft.WhiteChestPlate, 1);
				} else if (j > 50){
					this.dropItem(Wildycraft.WhiteLegs, 1);
				}else if (j > 25){
					this.dropItem(Wildycraft.WhiteHelmet, 1);
				}else {
					this.dropItem(Wildycraft.WhiteBoots, 1);
				}
			} else if (i > 60) {
				this.dropItem(Items.iron_helmet, 1);
			} else if (i > 58) {
				this.dropItem(Items.pumpkin_seeds, 2);
			} else if (i > 50) {
				this.dropItem(Item.getItemFromBlock(Blocks.iron_ore), 2);
			} else if (i > 40) {
				double j = rand.nextInt(100);
				if (j > 40){
					this.dropItem(Wildycraft.waterRune, 25);
				} else {
					this.dropItem(Wildycraft.bodyRune, 12);
				}
			} else if (i > 20) {
				this.dropItem(Wildycraft.whitesword, 1);
			} else if (i > 15) {
				double j = rand.nextInt(100);
				if (j > 40){
					this.dropItem(Wildycraft.sapphire, 1);
				} else {
					this.dropItem(Wildycraft.ruby, 1);
				}
			} else if (i > 10) {
