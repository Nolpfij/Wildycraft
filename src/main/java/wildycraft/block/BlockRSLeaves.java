...
public class BlockRSLeaves extends BlockLeavesBase implements IShearable
{
    public static final String[] LEAF_TYPES = new String[] {"jungle"};
    public static final String[][] field_94396_b = new String[][] {{"leaves_jungle"}, {"leaves_jungle_opaque"}};
                ...
                if (par1World.checkChunksExist(par2 - i1, par3 - i1, par4 - i1, par2 + i1, par3 + i1, par4 + i1))
                {
                    ...
                    for (l1 = 1; l1 <= 4; ++l1)
                    {
                        for (i2 = -b0; i2 <= b0; ++i2)
                        {
                            for (j2 = -b0; j2 <= b0; ++j2)
                            {
                                for (ka2 = -b0; ka2 <= b0; ++ka2)
                                {
                                    if (this.adjacentTreeBlocks[(i2 + k1) * j1 + (j2 + k1) * b1 + ka2 + k1] == l1 - 1)
                                    {
                                        if (this.adjacentTreeBlocks[(i2 + k1 - 1) * j1 + (j2 + k1) * b1 + ka2 + k1] == -2)
                                        {
                                            this.adjacentTreeBlocks[(i2 + k1 - 1) * j1 + (j2 + k1) * b1 + ka2 + k1] = l1;
                                        }
                                        ...
                                        if (this.adjacentTreeBlocks[(i2 + k1) * j1 + (j2 + k1 - 1) * b1 + ka2 + k1] == -2)
                                        {
                                            this.adjacentTreeBlocks[(i2 + k1) * j1 + (j2 + k1 - 1) * b1 + ka2 + k1] = l1;
                                        }
                                        ...
                                        if (this.adjacentTreeBlocks[(i2 + k1) * j1 + (j2 + k1) * b1 + (ka2 + k1 - 1)] == -2)
                                        {
                                            this.adjacentTreeBlocks[(i2 + k1) * j1 + (j2 + k1) * b1 + (ka2 + k1 - 1)] = l1;
                                        }
    ...
    {
        return Item.getItemFromBlock(Wildycraft.magicSapling);
    }
    ...
    public void dropBlockAsItemWithChance(World par1World, int par2, int par3, int par4, int par5, float par6, int par7)
    {
        if (!par1World.isRemote)
        {
            ...
            if (par1World.rand.nextInt(j1) == 0)
            {
                ...
                this.dropBlockAsItem(par1World, par2, par3, par4, new ItemStack(Wildycraft.magicSapling, 1, this.damageDropped(par5)));
            }
            ...
            {
                this.dropBlockAsItem(par1World, par2, par3, par4, new ItemStack(Wildycraft.birdsNest, 1, 0));
            }
        }
    }
