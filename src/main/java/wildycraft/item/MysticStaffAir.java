...
public class MysticStaffAir extends ItemBow {
	public MysticStaffAir() {
		super();
		this.maxStackSize = 1;
    ...
    public IIcon getIcon(ItemStack stack,int renderPass, EntityPlayer player, ItemStack usingItem, int useRemaining){
    	if (usingItem != null && usingItem.getItem() == Wildycraft.MysticAirStaff)
        {
	...
	public void onPlayerStoppedUsing(ItemStack par1ItemStack, World par2World,
		...
		for (int i = 0; i < 15; i++) {
			if (var5
				...
				// EntityArrow var81 = new EntityArrow(par2World,
				// par3EntityPlayer, var7 * 2.0F);
				EntityAirBlast var8 = new EntityAirBlast(par2World,
						par3EntityPlayer, var7);
				...
				if (var9 > 0) {
					var8.setDamage(var8.getDamage() + (double) var9 * 0.5D
							+ 0.5D);
				}
				...
				/*
				 * var8.setFire(100); if
				 * (EnchantmentHelper.getEnchantmentLevel(Enchantment
				 * .flame.effectId, par1ItemStack) > 0) { var8.setFire(100); }
				 */

				// Damages the item with 1
				par1ItemStack.damageItem(1, par3EntityPlayer);
				par2World.playSoundAtEntity(par3EntityPlayer, "random.breath",
				...
				if (var5) {
					...
					if (i%3 == 0){
					par3EntityPlayer.inventory
							.consumeInventoryItem(Wildycraft.airRune);
					}
				}
